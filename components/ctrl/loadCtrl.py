...
class LoadCtrl(DevCtrl):
    ...
    def __init__(self, name, dev, ctrl, host):
        ...
        if self.persistence != None:
            self.watchlist += ["devData", "devDataUpdate", "predictor", "lastPredictionUpdate", "history",
                               "predictionPlanning", "predictionDeviation"]
    ...
    def timeTick(self, time, deltatime=0):
        ...
        if time % self.timeBase == 0:
            ...
            i = int((time - (time % self.timeBase)) / ((
                                                                   self.predictionDeviationWindow * self.timeBase) / self.predictionDeviationWindow)) % self.predictionDeviationWindow
        ...
        if self.useEventControl and (self.lastPredictionUpdate + random.randint(self.predictionUpdateInterval[0],
                                                                                self.predictionUpdateInterval[
                                                                                    1])) <= time and not self.updatingPrediction:
    ...
    def doPlanning(self, signal, requireImprovement=True):
        ...
        if self.predictionPlanningTime < time:
            ...
            for c in self.commodities:
                p[c] = self.doPrediction(time - (time % timeBase),
                                         time - (time % timeBase) + timeBase * len(signal.desired[c]))
    ...
    def doPrediction(self, startTime, endTime, adapt=False):
        ...
        if self.predictionAdaption and not self.perfectPredictions and adapt:
            ...
            avg = max(0.5,
                      min(avg.real, 1.5))  # For now we add bounds to make sure errors don't have too severe impacts
    ...
    def initializePredictors(self):
        ...
        if self.perfectPredictions:
            ...
            self.givenPredictionReader = ClientCsvReader(dataSource=self.givenPrediction,
                                                         timeBase=self.devData['timeBase'],
                                                         column=self.devData['column'],
                                                         timeOffset=self.devData['timeOffset'], host=self.host)

            if self.intradayPrediction != None:
                self.intradayPredictionReader = ClientCsvReader(dataSource=self.intradayPrediction,
                                                                timeBase=self.devData['timeBase'],
                                                                column=self.devData['column'],
                                                                timeOffset=self.devData['timeOffset'], host=self.host)
            return
